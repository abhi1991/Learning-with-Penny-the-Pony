...
function scene:createScene( event )
    ...
    --STORY LINE
    local text = display.newText("Upang siya’y makabalik sa kaniyang bahay,", 100, 100, "Chinacat", 50)
    text.x = display.contentWidth * 0.5
    text.y = display.contentHeight * 0.20
    text:setTextColor(0,0,1)

    local text2 = display.newText("kailangan niyang malutas ang ilang problema  ", 100, 100, "Chinacat", 50)
    text2.x = display.contentWidth * 0.5
    text2.y = display.contentHeight * 0.27
    text2:setTextColor(0,0,1)
    
    local text3 = display.newText("na magsisilbing gabay sa kanyang daan pabalik...", 100, 100, "Chinacat", 50)
