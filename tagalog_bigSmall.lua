...
function scene:createScene( event )
    ...
    local instr = display.newImage("images/instructions/tagalog_identSizes.png")
        instr.x = display.contentWidth * 0.5
        instr.y = display.contentHeight * 0.5
    instr:scale(0.7,0.7)
    external.intruct("tagSize")
    ...
    local animalNumText = display.newText("Hayop #: "..times, 0, 0, "Chinacat", 50)
    ...
    local function instructionFunction()
        ...
        if r == 1 then
            instruction.text = "Piliin ang MALAKING na hayop"
            external.feeds("malaki")
            big = 1
        else
            instruction.text = "Piliin ang MALIIT na hayop"
            external.feeds("maliit")
            big = 0
        end
    end
    ...
    local function nextFunction()
        times = times + 1
        animalNumText.text = "Hayop #: "..times
        nextButton.alpha = 0
        chooseAnimal()
    end
    ...
    local function restartGame()
        storyboard.gotoScene("refresh_tagalog_bigSmall", "fade", 300)
    end
...
function scene:exitScene( event )
    external.setPreviousScene("tagalog_bigSmall")
end
