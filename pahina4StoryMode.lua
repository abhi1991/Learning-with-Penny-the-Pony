...
function scene:createScene( event )
    ...
    local background = display.newImageRect("storyline/5.jpg", display.contentWidth, display.contentHeight)
     ...
     local nextButton = widget.newButton
    {
        ...
        onRelease = function()
            storyboard.gotoScene( "pahina5StoryMode", "flip", 400 )
        end,
    }
    ...
    --STORY LINE
    local text = display.newText("Isang araw, bilang isang mapaglarong little pony,", 100, 100, "Chinacat", 40)
    text.x = display.contentWidth * 0.5
    text.y = display.contentHeight * 0.70
